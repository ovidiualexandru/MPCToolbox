...
for i = 1:N
    %% Get next command
    tic;
    [ue, Xe,fval,EXITFLAG] = lmpc_condensed(...
        A, B, Q, R, Nc, du, dx, [], [], [], [], x, xref, [], ue);
    if EXITFLAG < 0
...
grid on
title('Input u');
line([0;N],[du(1);du(1)], 'LineStyle', '--', 'Color', [1 0 0]);%UB
line([0;N],[du(2);du(2)], 'LineStyle', '--', 'Color', [1 0 0]);%LB
...
line([0;N],[dx(1,1);dx(1,1)], 'LineStyle', '--', 'Color', [1 0 0]);%UB
line([0;N],[dx(2,1);dx(2,1)], 'LineStyle', '--', 'Color', [1 0 0]);%LB
line([dist_k;dist_k],get(gca,'YLim'), 'LineStyle', '--', 'Color', [0 1 0]);
...
grid on
title('Arm speed x_2');
line([0;N],[dx(1,2);dx(1,2)], 'LineStyle', '--', 'Color', [1 0 0]);%UB
line([0;N],[dx(2,2);dx(2,2)], 'LineStyle', '--', 'Color', [1 0 0]);%LB
line([dist_k;dist_k],get(gca,'YLim'), 'LineStyle', '--', 'Color', [0 1 0]);
...
line([0;N],[dx(1,3);dx(1,3)], 'LineStyle', '--', 'Color', [1 0 0]);%UB
line([0;N],[dx(2,3);dx(2,3)], 'LineStyle', '--', 'Color', [1 0 0]);%LB
line([dist_k;dist_k],get(gca,'YLim'), 'LineStyle', '--', 'Color', [0 1 0]);
...
line([0;N],[dx(1,4);dx(1,4)], 'LineStyle', '--', 'Color', [1 0 0]);%UB
line([0;N],[dx(2,4);dx(2,4)], 'LineStyle', '--', 'Color', [1 0 0]);%LB
line([dist_k;dist_k],get(gca,'YLim'), 'LineStyle', '--', 'Color', [0 1 0]);

axes('Position',[0.25 0.9 0.5 0.1],...
    'Box','off','Visible','off','Units','normalized', 'clipping' , 'off');
titlestring = sprintf('Run with N=%d, Nc = %d',N,Nc);
text(0.5, 1, ['\bf ' titlestring],...
    'HorizontalAlignment','center','VerticalAlignment', 'top')

plot_ft(FVAL,TEVAL,'LMPC(condensed) Inverted Pendulum Performance',figure);
